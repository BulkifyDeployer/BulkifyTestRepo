...
import { getBalance, getNativeBalance, balanceQueryKey, getConfig } from "utils"
import { BigNumber } from "ethers"

export function useNativeBalance(tokenSymbol, chainId, account, blockNumber) {
...
const QueryBalanceBySymbol = params => async () => {
  const {
    accountToQuery,
    chainIdToQuery,
    tokenSymbolToQuery,
  ...
  if (tokenInfo.isNative) {
    return getNativeBalance(chainIdToQuery, accountToQuery, blockNumberToQuery)
  } else {
    return getBalance(
      chainIdToQuery,
      accountToQuery,
 ...
 */
export function useBalanceBySymbol(tokenSymbol, chainId, account, blockNumber) {
  const { account: connectedAccount } = useConnection()
  const chainIdToQuery = chainId
  const tokenSymbolToQuery = tokenSymbol
  const accountToQuery = account ?? connectedAccount
