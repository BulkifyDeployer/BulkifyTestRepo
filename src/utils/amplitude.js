...
import { ConvertDecimals } from "./convertdemicals"
import {
  formatUnits,
  formatEther,
  formatWeiPct,
...
export function trackConnectWalletButtonClicked(section) {
...
export function trackIfWalletSelected(walletStates, previousConnection) {
...
export function trackFromChainChanged(chainId, isDefault) {
...
export function trackToChainChanged(chainId, isDefault) {
  if (Number.isNaN(chainId)) return Promise.resolve()
  const chain = getChainInfo(chainId)
...
export function trackQuickSwap(section) {
...
export function trackTokenChanged(tokenSymbol, isDefault) {
...
export function trackDisconnectWalletButtonClicked(section) {
...
export function setUserId(walletAddress) {
  if (!walletAddress) {
    return
  }
...
export function identifyUserWallet(connectedWallet) {
  if (!connectedWallet) {
    return
  }
...
export function identifyWalletChainId(chainId) {
  const identifyObj = new Identify()
...
export function trackMaxButtonClicked(section) {
...
export function generateTransferQuote(
  ...
  const usdEquivalentString = wei =>
    formatEther(usdEquivalent(wei)).replaceAll(",", "")
  const formatWeiEtherPct = wei => formatWeiPct(wei).toString()
...
export function reportTokenBalance(chainId, balance, symbol) {
...
export function reportTotalWalletUsdBalance(totalBalance) {
